...
from keras.layers import TimeDistributed, Reshape, LSTM, GlobalAveragePooling1D
...
def define_CNN_LSTM(input_shape, n_keypoints):
...
def train_and_evaluate():
    ...
    model = define_CNN_LSTM(X_train[0].shape, N_KEYPOINTS)
    model.summary()

    history = model.fit(X_train, y_train, validation_split=0.2, 
                        batch_size=BATCH_SIZE, epochs=EPOCHS)

    model.save(os.path.join(MODEL_DIR, "trained_model_lstm.h5"))
